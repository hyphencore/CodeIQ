...
*/
	 ...
	 */
	public int getMaxValidAreaSize() {
		int x, y, x2, xCount, yCount, strY, endY, maxS = 0, preStatY, preStatX;

		for (x=0 ; x<xSize ; x++) {
			xCount = 0; yCount = 0; strY = -1; endY = -1; preStatY = BATSU; preStatX = BATSU;
			for (y=0 ; y<ySize ; y++) {
				if (area[y][x] != BATSU) {
					if (preStatY == BATSU) xCount = 0; yCount = 0; strY = -1; endY = -1;
					if (strY < 0) strY = y;
					yCount++;
					endY = y;
				} else {
